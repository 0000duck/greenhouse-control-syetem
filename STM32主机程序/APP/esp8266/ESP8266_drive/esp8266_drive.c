...
void getdata(char *src,char *subsrc,u8 *data){
    char *p = strstr(src,subsrc);
    if((*(p+2))&&(*(p+3))){
         (*data)=(*(p+2)-48)*10+(*(p+3)-48);
    }
}
...
char Hand(unsigned char *a)
{ 
    if(strstr(ESP8266_Fram_Record_Struct .Data_RX_BUF,a)!=NULL)
	    return 1;
	else
		return 0;
}
...
{
	  unsigned char k;
    for(k=ESP8266_Fram_Record_Struct .InfBit .FramLength-12;k<(ESP8266_Fram_Record_Struct .InfBit .FramLength+1);k++)    
	    {
			ESP8266_Fram_Record_Struct .Data_RX_BUF[k] = 0;
		}
		//ESP8266_Fram_Record_Struct .InfBit .FramFinishFlag=1;
    ESP8266_Fram_Record_Struct .InfBit .FramLength = ESP8266_Fram_Record_Struct .InfBit .FramLength-12;	//从新开始接收新的数据包                
}
/*
void CLR_Buf(void)
{
	  unsigned char k;
    for(k=ESP8266_Fram_Record_Struct .InfBit .FramLength-9;k<(ESP8266_Fram_Record_Struct .InfBit .FramLength+1);k++)    
	    {
			ESP8266_Fram_Record_Struct .Data_RX_BUF[k] = 0;
		}
		//ESP8266_Fram_Record_Struct .InfBit .FramFinishFlag=1;
    ESP8266_Fram_Record_Struct .InfBit .FramLength = ESP8266_Fram_Record_Struct .InfBit .FramLength-9;	//从新开始接收新的数据包                
}*/

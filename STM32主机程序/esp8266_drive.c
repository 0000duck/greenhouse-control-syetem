...
{	
    ...
    if(USART_GetITStatus( USART2, USART_IT_RXNE ) != RESET )
    {
        ...
        if(ucCh=='S')		  	                         //如果是S，表示是命令信息的起始位
        {
           ESP8266_Fram_Record_Struct .InfBit .FramLength=0X01;
        }
        else if(ucCh=='E')		                         //如果E，表示是命令信息传送的结束位
        {
            if(strcmp("open_led1",(char *)ESP8266_Fram_Record_Struct .Data_RX_BUF)==0)
            {
               led1=0;	   //点亮LED0
            }
            else if(strcmp("close_led1",(char *)ESP8266_Fram_Record_Struct .Data_RX_BUF)==0)
            {
               led1=1;	   //关灭LED0
            }
            else if(strcmp("open_led2",(char *)ESP8266_Fram_Record_Struct .Data_RX_BUF)==0)
            {
                led2=0;	   //点亮LED1
            }
            else if(strcmp("close_led2",(char *)ESP8266_Fram_Record_Struct .Data_RX_BUF)==0)
            {
                led2=1;	   //关灭LED1
            }
            else if(strcmp("open_led3",(char *)ESP8266_Fram_Record_Struct .Data_RX_BUF)==0)
            {
                led3=0;	   //点亮LED2
            }
            else if(strcmp("close_led3",(char *)ESP8266_Fram_Record_Struct .Data_RX_BUF)==0)
            {
                led3=1;	   //关灭LED2
            }
            else if(strcmp("open_beep",(char *)ESP8266_Fram_Record_Struct .Data_RX_BUF)==0)
            {
                beep=1; 	   //蜂鸣器响
            }
            else if(strcmp("close_beep",(char *)ESP8266_Fram_Record_Struct .Data_RX_BUF)==0)
            {
                beep=0; 	   //蜂鸣器不响
            }
            else if(strcmp("app_mode1",(char *)ESP8266_Fram_Record_Struct .Data_RX_BUF)==0)
            {
                    APP_mode=0;
            ...
            else if(strcmp("app_mode2",(char *)ESP8266_Fram_Record_Struct .Data_RX_BUF)==0)
            {
                    APP_mode=1;
    ...
    /*	else if(ESP8266_Fram_Record_Struct .Data_RX_BUF[0]=='T')   //时间校准
            {
            ...
            //	RTC_DateStruct.RTC_Date= (receive_str[7]-'0')*10 + (receive_str[8]-'0');
                    ...
                    //ESP8266_SendString(DISABLE,"correct_OK",strlen("correct_OK"),Multiple_ID_1);
            }
    */
            ...
            for(i=0;i<32;i++)
               ESP8266_Fram_Record_Struct .Data_RX_BUF[i]=0x00;
            ESP8266_Fram_Record_Struct .InfBit .FramLength=0;
        }

        else if((ESP8266_Fram_Record_Struct .InfBit .FramLength >0)&&ESP8266_Fram_Record_Struct .InfBit .FramLength < ( RX_BUF_MAX_LEN - 1 ) )
        {
            //预留1个字节写结束符
            ESP8266_Fram_Record_Struct .Data_RX_BUF[ ESP8266_Fram_Record_Struct .InfBit .FramLength -1]  = ucCh;

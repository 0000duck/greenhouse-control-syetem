...
unsigned char i = 0; 
byte temperatureStr[8]="1T";
byte humidityStr[8]="1H";
...
{	
  ...
  U1SendString("OK,ESP8266 has been set as Station Mode\r\n");	
	...
	*/
	...
	LED2 = 0;
	CLR_Buf();              
  U1SendString("OK,Succeed establish connection with WiFi AP PUZHONG\r\n");	
...
/*	while(!Hand("OK"))                                   // 建立TCP 服务器，并开放端口8888
  ...
  while (1)		                                        // 主循环  
  {
      ...
      LCD_P6x8Str(40,5,temp_buf);  
			...
			strcpy(temperatureStr,"1T");
		  strcpy(humidityStr,"1H");	
